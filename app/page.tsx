...
export default function Home() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.reply) setChat((prev) => [...prev, { role: "ai", content: data.reply.content }]);
      else if (data.error) setChat((prev) => [...prev, { role: "ai", content: "Error: " + data.error }]);
    } catch {
      ...
      <div className="border rounded p-4 h-96 overflow-y-auto mb-4 bg-gray-50">
        {chat.map((msg, idx) => (
          <div key={idx} className={`mb-2 ${msg.role === "user" ? "text-right" : "text-left"}`}>
            <span className={`inline-block px-3 py-1 rounded ${msg.role === "user" ? "bg-blue-500 text-white" : "bg-gray-300"}`}>
              {msg.content}
            </span>
          </div>
        ))}
